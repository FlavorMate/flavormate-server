...
public class S01_InitDatabaseScript implements AScript {
  ...
  private void initializeCategoryGroups() throws Exception {
    try {
      ...
      CategoryGroup[] categoryGroupsArr =
          JSONUtils.mapper.readValue(categoryGroupsFile.getInputStream(), CategoryGroup[].class);
  ...
  private void initializeCategories() throws Exception {
    try {
      ...
      var categoryList = JSONUtils.mapper.readValue(categoriesFile.getInputStream(), typeReference);
      ...
      var categories =
          categoryList.stream()
              .map(map -> JSONUtils.mapper.convertValue(map, Category.class))
              .toList();
  ...
  private void initializeCategoryLocalizations() throws Exception {
    try {
      ...
      for (var categoryLocalizationFile : categoryLocalizationsFiles) {
        ...
        var localizations =
            JSONUtils.mapper.readValue(categoryLocalizationFile.getInputStream(), typeReference);
        ...
        for (var localization : localizations.entrySet()) {
          ...
          var categoryLocalization =
              CategoryLocalization.builder()
                  .id(new LocalizationId(category.getId(), language))
                  .category(category)
                  .value(localization.getValue())
                  .build();
      ...
      for (var categoryGroupLocalizationFile : categoryGroupsLocalizationsFiles) {
        ...
        var localizations =
            JSONUtils.mapper.readValue(
                categoryGroupLocalizationFile.getInputStream(), typeReference);
        ...
        for (var localization : localizations.entrySet()) {
          ...
          var categoryGroupLocalization =
              CategoryGroupLocalization.builder()
                  .id(new LocalizationId(categoryGroup.getId(), language))
