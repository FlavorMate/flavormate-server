 ...
 * "flavormate.features.registration.enabled" is set to true, this service will be active. It
 * handles the logic of user account creation, including checks for existing email or username,
 * password encoding, and role assignment.
 *
 * <p>Utilizes the following repositories: - AccountRepository: For checking existence and saving
 * Account entities. - AuthorRepository: For saving new Author entities associated with the created
 * Account. - RoleRepository: For fetching user roles.
 *
 * <p>This service is also dependent on a password encoder to hash user passwords.
 *
 * <p>The create method throws CustomException on error conditions such as conflict when the email
 * or username already exists.
 *
 * <p>Dependencies: - AccountRepository accountRepository - AuthorRepository authorRepository -
 * RoleRepository roleRepository - PasswordEncoder passwordEncoder
 */
@ConditionalOnProperty(
    prefix = "flavormate.features.registration",
    value = "enabled",
    havingValue = "true")
@Service
...
public class SelfServiceRegistrationService {
  ...
  /**
   * Creates a new account based on the provided account draft. The method checks if an account with
   * the same email or username already exists and throws a ConflictException if true. It then
   * encodes the password, assigns a default role, and saves the new account. Additionally, an
   * associated author entity is created and saved.
   *
