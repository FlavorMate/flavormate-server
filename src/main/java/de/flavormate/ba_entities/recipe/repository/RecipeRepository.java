...
public interface RecipeRepository extends JpaRepository<Recipe, Long> {
  ...
  @Query(
      "select r from Recipe r join r.books b where b.id = :id and b.owner.account.username ="
          + " :username")
  Page<Recipe> findByBookPrivate(
      @Param("id") Long id, @Param("username") String username, Pageable pageable);
  ...
  @Query(
      "select r from Recipe r where lower(r.label) like lower(concat('%', :label, '%')) and r.diet"
          + " in :diet")
  ...
  @Query(
      nativeQuery = true,
      value =
          "SELECT * FROM RECIPES WHERE diet = ANY (?1) AND course LIKE ?2 ORDER BY RANDOM() LIMIT"
              + " ?3")
  ...
  @Query(
      "SELECT r FROM Recipe r Join r.ingredientGroups ig Join ig.ingredients i WHERE i.id ="
          + " :ingredient")
