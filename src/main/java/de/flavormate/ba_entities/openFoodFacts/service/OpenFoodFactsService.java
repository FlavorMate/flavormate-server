...
@ConditionalOnProperty(
	{"flavormate.features.open-food-facts.enabled", "flavormate.features.recovery.enabled"})
@Service
...
public class OpenFoodFactsService {
	...
	public NutritionDraft fetchProduct(String barcode) {
		...
		var response =
			restTemplate.exchange(uri, HttpMethod.GET, entity, OpenFoodFactsResponse.class);

		var nutriments = Optional.ofNullable(response.getBody()).map(OpenFoodFactsResponse::product)
			.map(OpenFoodFactsProduct::nutriments).orElse(null);

		if (nutriments == null)
			return null;

		return new NutritionDraft(barcode, nutriments.carbohydrates(), nutriments.energy_kcal(),
			nutriments.fat(), nutriments.saturatedFat(), nutriments.sugars(), nutriments.fiber(),
			nutriments.proteins(), nutriments.salt(), nutriments.sodium());
	}
}
