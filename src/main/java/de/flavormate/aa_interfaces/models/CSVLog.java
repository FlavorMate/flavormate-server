...
/**
 * CSVLog is a utility class for creating and writing CSV log files. It maintains a list of rows
 * representing the CSV content and provides methods to add rows, write the CSV file to a specified
 * path, and check if the log contains any content.
 */
public class CSVLog {

  /** The name of the CSV log file, dynamically generated with a timestamp to ensure uniqueness. */
  private final String name;

  /**
   * Stores the rows of the CSV log. Each row is represented as a list of strings. The first entry
   * is typically the header, followed by data rows.
   */
  ...
  /**
   * Creates a new CSV log with the specified name and headers. The name is appended with the
   * current timestamp to ensure uniqueness. The headers are added as the first row in the CSV log.
   *
   ...
   *
   * @param row the list of strings representing the row to be added. The number of elements in the
   *     list must match the number of fields specified in the header.
   * @throws IllegalArgumentException if the number of elements in the row does not match the number
   *     of fields.
   */
  ...
  public boolean writeFile(Path migration) {
    try {
      ...
      var contents =
          rows.stream()
              .map(row -> String.join(";", row.stream().map(CSVLog::cleanString).toList()))
              .toList();
