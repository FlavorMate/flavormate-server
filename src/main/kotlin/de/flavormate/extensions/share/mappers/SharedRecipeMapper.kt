...
class SharedRecipeMapper(
  ...
  override fun mapNotNullBasic(input: RecipeEntity): SharedRecipe {
    return SharedRecipe(
      ...
      prepTime = input.prepTime.takeIf { it != Duration.ZERO }?.let { mapDuration(it) },
      cookTime = input.cookTime.takeIf { it != Duration.ZERO }?.let { mapDuration(it) },
      restTime = input.restTime.takeIf { it != Duration.ZERO }?.let { mapDuration(it) },
