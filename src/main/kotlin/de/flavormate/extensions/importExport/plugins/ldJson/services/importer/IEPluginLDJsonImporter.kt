...
package de.flavormate.extensions.importExport.plugins.ldJson.services.importer
...
class IEPluginLDJsonImporter(
  ...
  fun import(input: LDJsonRecipe): IERecipeDraft {
    ...
    return IERecipeDraft(
      ...
      files =
        input.images.mapNotNull {
          downloadService.downloadFile(URIBuilder(it), context.maxImageSize)?.toFile()
        },
      url = input.url,
      language = lang,
    )
